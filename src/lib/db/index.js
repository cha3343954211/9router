...
export async function exportDb() {
  ...
  const providerConnectionRows = await db.all(`SELECT * FROM providerConnections`);
  const providerNodeRows = await db.all(`SELECT * FROM providerNodes`);
  const proxyPoolRows = await db.all(`SELECT * FROM proxyPools`);
  const apiKeyRows = await db.all(`SELECT * FROM apiKeys`);
  const comboRows = await db.all(`SELECT * FROM combos`);

  const out = {
    settings: await exportSettings(),
    providerConnections: providerConnectionRows.map((r) => ({ ...parseJson(r.data, {}), id: r.id, provider: r.provider, authType: r.authType, name: r.name, email: r.email, priority: r.priority, isActive: r.isActive === 1, createdAt: r.createdAt, updatedAt: r.updatedAt })),
    ...
    proxyPools: proxyPoolRows.map((r) => ({ ...parseJson(r.data, {}), id: r.id, isActive: r.isActive === 1, testStatus: r.testStatus, createdAt: r.createdAt, updatedAt: r.updatedAt })),
    apiKeys: apiKeyRows.map((r) => ({ id: r.id, key: r.key, name: r.name, machineId: r.machineId, isActive: r.isActive === 1, createdAt: r.createdAt })),
